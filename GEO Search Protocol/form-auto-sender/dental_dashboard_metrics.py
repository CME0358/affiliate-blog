...
ARI_ITEMS: tuple[tuple[str, str], ...] = (
    ...
    ("LocalBusiness", "Dentist / LocalBusiness 等"),
...
@dataclass
class DentalDashboardStats:
    start_date: str
    completed_count: int
    today_ward: str
    today_status: str
    total_clinics: int
    clinics_with_url: int
...
def load_state(vault_root: Path) -> dict[str, Any]:
    path = crawler_root(vault_root) / "data/ward_schedule_state.json"
    if not path.exists():
        return {"start_date": "2026-06-17", "completed": {}, "runs": []}
...
def collect_stats(vault_root: Path, today: date) -> DentalDashboardStats:
    state = load_state(vault_root)
    start = date.fromisoformat(state.get("start_date", "2026-06-17"))
    ...
    master = crawler_root(vault_root) / "data/master"
    clinic_rows = _read_csv_rows(master / "clinic_list_all.csv")
    ...
    url_col = "HP URL" if clinic_rows and "HP URL" in clinic_rows[0] else "url"
    total_clinics = len(clinic_rows)
    clinics_with_url = sum(1 for r in clinic_rows if (r.get(url_col) or "").strip())
    ...
    return DentalDashboardStats(
        start_date=start.isoformat(),
        completed_count=completed_count,
        today_ward=today_ward,
        today_status=today_status,
        total_clinics=total_clinics,
        clinics_with_url=clinics_with_url,
...
def build_dental_section_html(vault_root: Path, today: date, generated_ts: str) -> str:
    ...
    kpis = "\n".join(
        [
            kpi("完了区数", f"{stats.completed_count}/23", "2026-06-17〜07-09"),
            kpi("本日の区", stats.today_ward, stats.today_status),
            kpi("累計クリニック", stats.total_clinics, f"HP URLあり {stats.clinics_with_url}"),
            kpi("ARI 平均", f"{stats.ari_avg:.1f}", f"{stats.ari_count}件・100点満点"),
            kpi("予約システム検出", stats.reservation_detected, "Apotool / RESERVA 等"),
            kpi(
                "次回実行",
                "18:00",
    ...
    return f"""<section id="dental-crawler" class="tab-panel" role="tabpanel" aria-labelledby="tab-dental-crawler" hidden>
      <div class="section-head">
        <h2>歯科クローラー · Agent Readiness Index</h2>
        <span class="section-tag">23区 · 1日1区 · 18:00</span>
      </div>
      <p class="expo-note">
        <strong>データソース:</strong> 厚生労働省 医療情報ネット（ナビイ）· Google Maps 不使用。
      ...
      <div class="card">
        <table>
          ...
          <tbody>
            <tr>
              <td>日次スケジューラ</td>
              <td><code>{escape_html(crawler_rel)}/daily_ward_diagnosis.py</code></td>
            </tr>
            <tr>
              <td>リスト取得</td>
              <td><code>{escape_html(crawler_rel)}/collect_clinic_list.py</code></td>
            </tr>
            <tr>
              <td>予約システム判定</td>
            ...
            <tr>
              <td>区別出力</td>
              <td><code>{escape_html(crawler_rel)}/output/YYYY-MM-DD_区名/</code></td>
            </tr>
            <tr>
              <td>全区統合 CSV</td>
              <td><code>{escape_html(crawler_rel)}/data/master/</code></td>
            </tr>
            <tr>
              <td>進捗 state</td>
              <td><code>{escape_html(crawler_rel)}/data/ward_schedule_state.json</code></td>
            </tr>
            <tr>
              <td>実行ログ</td>
              <td><code>{escape_html(crawler_rel)}/data/daily_ward.log</code></td>
            </tr>
          </tbody>
        </table>
      </div>
