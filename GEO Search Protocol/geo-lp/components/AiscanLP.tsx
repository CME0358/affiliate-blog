...
// ── CTA Button ───────────────────────────────────────────────────────────
function CTAButton({ onClick, size = 'lg', label, variant = 'A' }: {
    ...
    >
      ...
      <span style={{ textAlign: 'center', lineHeight: 1.5 }}>
        {label
          ...
          : <>なぜ競合はAIに出て、自社は出ていないのか<br />【診断結果を受け取る】</>
        }
      </span>
    </button>
  )
...
export default function AiscanLP({ headline }: AiscanLPProps = {}) {
  ...
  return (
    <>
      {modalOpen && <FormModal onClose={() => setModalOpen(false)} variant="A" />}
      ...
      {/* ── TICKER ── */}
      <div
        onClick={() => { trackCTAClick('A', 'ticker_cta'); setModalOpen(true) }}
        style={{
          ...
          <div style={{
            ...
            <div className="animate-fadeInUp delay-1" style={{
              ...
              <span style={{ fontSize: 17, color: '#FF8AAD', fontWeight: 600 }}>
                AI検索時代の新常識 — GEO (Generative Engine Optimization)
              </span>
            </div>
            ...
            <p className="animate-fadeInUp delay-3" style={{
              ...
              出てこなければ、<strong style={{ color: '#FFFFFF' }}>比較対象にすら入りません。</strong><br />
              まずは4つのAIでの認識状況を無料で可視化します。
            </p>

            {/* CTA */}
            <div className="animate-fadeInUp delay-4" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
            ...
            <div
              ...
              onClick={() => { trackCTAClick('A', 'ai_logos_cta'); setModalOpen(true) }}
              style={{
